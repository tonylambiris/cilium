...
do_ct:
	...
	case CT_NEW:
		...
		if (svc_hit) {
			...
			ret = ct_create4(&CT_MAP4, &tuple, skb, CT_EGRESS, &ct_state_new, false);
			if (IS_ERR(ret))
				return TC_ACT_OK;
		}
		break;
...
static inline int __inline__ ipv4_policy(struct __sk_buff *skb, int ifindex, __u32 src_label)
{
	...
	cilium_trace(skb, DBG_GENERIC, 20, 30);
	...
	cilium_trace(skb, DBG_GENERIC, 20, 40);
	ret = ct_lookup4(&CT_MAP4, &tuple, skb, l4_off, CT_INGRESS, &ct_state);
	if (ret < 0)
		return ret;

	cilium_trace(skb, DBG_GENERIC, 20, 50);
...
__section_tail(CILIUM_MAP_RES_POLICY, SECLABEL) int handle_policy(struct __sk_buff *skb)
{
	...
	cilium_trace(skb, DBG_GENERIC, 20, 20);
